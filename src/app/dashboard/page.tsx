...
import type { OrgGamificationState } from '@/lib/gamification'
...
function DashboardInner() {
  ...
  const [factorVersionYear, setFactorVersionYear] = useState('2024')

  useEffect(() => {
    const savedProfile = localStorage.getItem('org_profile')
    if (savedProfile) {
      const p = JSON.parse(savedProfile)
      if (p.name) fetch(`/api/achievements?org=${encodeURIComponent(p.name)}`).then(r => r.json()).then(d => d.state && setGameState(d.state))
    }
  ...
  async function handleCalculate(data: {
    ...
    try {
      ...
      // Refresh gamification state after each calc and check for new achievements
      const calcOrg = data.organisation_name
      fetch(`/api/achievements?org=${encodeURIComponent(calcOrg)}`).then(r => r.json()).then(d => {
        if (d.state) {
          const prev = gameState?.unlocked_count ?? 0
          const next = d.state.unlocked_count
          if (next > prev) {
            const newest = d.state.achievements.find((a: { unlocked: boolean }) => a.unlocked)
            if (newest) setNewAchievement(newest.name)
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <nav style={{ background: 'var(--white)', borderBottom: '1px solid var(--border)', padding: '0 2rem', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50, boxShadow: 'var(--shadow-sm)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          {gameState && (
            <button onClick={() => router.push(`/achievements?org=${encodeURIComponent(profile?.name ?? '')}`)} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', background: 'var(--bg)', border: '1px solid var(--border)', padding: '0.25rem 0.6rem', borderRadius: 20, cursor: 'pointer' }}>
              <div style={{ width: 18, height: 18, borderRadius: 4, background: gameState.level.colour, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.55rem', fontWeight: 800, color: '#fff' }}>{gameState.level.level}</div>
              <span style={{ ...mono, fontSize: '0.65rem', fontWeight: 700, color: 'var(--text-2)' }}>{gameState.xp.toLocaleString()} XP</span>
              <div style={{ width: 40, height: 4, background: 'var(--border)', borderRadius: 2, overflow: 'hidden' }}>
                <div style={{ width: `${gameState.xp_progress_pct}%`, height: '100%', background: gameState.level.colour }} />
              </div>
            </button>
          )}
          <button onClick={() => router.push('/history')} style={{ ...mono, fontSize: '0.72rem', color: 'var(--text-3)', background: 'none', border: '1px solid var(--border)', padding: '0.3rem 0.75rem', borderRadius: 'var(--radius-xs)', cursor: 'pointer' }}>History</button>
          <button onClick={() => router.push('/onboarding')} style={{ ...mono, fontSize: '0.72rem', color: 'var(--text-3)', background: 'none', border: '1px solid var(--border)', padding: '0.3rem 0.75rem', borderRadius: 'var(--radius-xs)', cursor: 'pointer' }}>{profile ? '⚙ Profile' : '+ Setup'}</button>
          <select value="" onChange={e => applyTestProfile(e.target.value)}
            ...
            {TEST_PROFILES.map((tp, i) => <option key={i} value={String(i)}>{tp.label}</option>)}
          </select>
        </div>
      </nav>
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        {!profile && (
          <div style={{ marginBottom: '1.25rem', padding: '0.875rem 1.25rem', background: 'var(--amber-light)', border: '1px solid rgba(184,122,0,0.2)', borderRadius: 'var(--radius-sm)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <p style={{ fontSize: '0.875rem', color: 'var(--amber)', fontWeight: 500 }}>◎ Set up your organisation profile to enable intensity metrics and site tracking</p>
            <button onClick={() => router.push('/onboarding')} style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--amber)', background: 'none', border: '1px solid currentColor', padding: '0.3rem 0.75rem', borderRadius: 'var(--radius-xs)', cursor: 'pointer', whiteSpace: 'nowrap' }}>Set up →</button>
          </div>
        )}

        {/* Profile bar */}
        {profile && (
          <div style={{ marginBottom: '1.25rem', padding: '0.75rem 1.25rem', background: 'var(--green-light)', border: '1px solid var(--border-strong)', borderRadius: 'var(--radius-sm)', display: 'flex', alignItems: 'center', gap: '1.5rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ marginBottom: '1.75rem' }}>
          <span style={{ ...mono, fontSize: '0.68rem', color: 'var(--green)', letterSpacing: '0.08em', textTransform: 'uppercase', background: 'var(--green-light)', padding: '0.2rem 0.6rem', borderRadius: 20, fontWeight: 500 }}>Scope 1 · 2 · 3</span>
          <h1 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, fontSize: '1.75rem', letterSpacing: '-0.02em', color: 'var(--text)', lineHeight: 1.2, marginTop: '0.5rem' }}>Emissions Calculator</h1>
          <p style={{ marginTop: '0.4rem', color: 'var(--text-3)', fontSize: '0.875rem' }}>DESNZ 2024 factors · GHG Protocol · Full audit trail for SECR disclosure</p>
        </div>

        {/* Layout */}
        <div style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: result ? '420px 1fr' : '520px' }}>
          <div>
            <InputForm key={mountKey} onCalculate={handleCalculate} loading={loading} profile={profile} template={template} factorVersionYear={factorVersionYear} onVersionChange={setFactorVersionYear} />
            {error && (
              <div style={{ marginTop: '0.75rem', padding: '0.75rem 1rem', background: 'var(--red-light)', color: 'var(--red)', border: '1px solid rgba(184,50,50,0.2)', borderRadius: 'var(--radius-sm)', ...mono, fontSize: '0.75rem' }}>
                {error}
              </div>
            )}
          </div>
      ...
      {/* Achievement toast */}
      {newAchievement && (
        <div style={{ position: 'fixed', bottom: '2rem', right: '2rem', background: '#141a12', color: '#fff', padding: '1rem 1.25rem', borderRadius: 'var(--radius)', boxShadow: '0 8px 32px rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', gap: '0.75rem', zIndex: 1000, animation: 'slideUp 0.4s ease', maxWidth: 320 }}>
          <div style={{ fontSize: '1.5rem' }}>🏆</div>
          <div>
            <p style={{ ...mono, fontSize: '0.6rem', color: 'rgba(255,255,255,0.5)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.2rem' }}>Achievement Unlocked!</p>
            <p style={{ fontWeight: 700, fontSize: '0.875rem', color: '#4ca66a' }}>{newAchievement}</p>
          </div>
        </div>
      )}

import { getFactor, getFactorVersion, isProvisional } from '@/lib/factors'
import type {
  ActivityInput, AuditEntry, CalculationResult,
  EmissionLineResult, GapItem, IntensityMetrics, SiteBreakdown
} from '@/types'
...
function buildWttInputs(inputs: ActivityInput[]): ActivityInput[] {
  return inputs.flatMap((input) => {
    ...
    return [{
      id: `${input.id}_wtt`,
      source_type: wttFactorId,
      factor_id: wttFactorId,
      quantity,
      unit: WTT_UNIT[wttFactorId],
      period_start: input.period_start,
      period_end: input.period_end,
      site: input.site,
      estimated: input.estimated,
      notes: `Auto WTT for ${input.factor_id}`,
    }]
  })
}
...
function buildSiteBreakdown(lines: EmissionLineResult[]): SiteBreakdown[] {
  ...
  lines.forEach((l) => {
    ...
    if (!siteMap[site]) siteMap[site] = { site, t_co2e: 0, scope_1: 0, scope_2: 0, scope_3: 0, line_count: 0 }
...
export function calculateLine(input: ActivityInput, version?: string): EmissionLineResult {
  const factor = getFactor(input.factor_id, version)
  const factor_version = getFactorVersion(version)
...
export function calculate(request: CalculationRequest): CalculationResult {
  const { organisation_name, reporting_period_start, reporting_period_end, inputs, intensity, factor_version_year } = request
  ...
  const lines      = allInputs.map(inp => calculateLine(inp, factor_version_year))

  const scope1 = lines.filter(l => l.scope === 1)
  const scope2 = lines.filter(l => l.scope === 2)
  const scope3 = lines.filter(l => l.scope === 3)
  const scope_1_t_co2e = round(scope1.reduce((a, l) => a + l.t_co2e, 0), 4)
  const scope_2_t_co2e = round(scope2.reduce((a, l) => a + l.t_co2e, 0), 4)
  const scope_3_t_co2e = round(scope3.reduce((a, l) => a + l.t_co2e, 0), 4)
  ...
  if (total_t_co2e > 0) {
    let wq = 0, wu = 0
    lines.forEach(l => {
      const w = l.t_co2e / total_t_co2e
      wq += (tierWeights[l.data_quality_tier] ?? 20) * (l.input.estimated ? 0.6 : 1.0) * w
      wu += ((tierUncertainty[l.data_quality_tier] ?? 50) + (l.input.estimated ? 15 : 0)) * w
    })
    data_quality_score = Math.round(wq)
  ...
  const gaps: GapItem[] = []
  if (scope2.filter(l => !l.input.notes?.startsWith('Auto WTT')).length === 0)
    gaps.push({ code: 'MISSING_SCOPE_2', severity: 'critical', message: 'No Scope 2 electricity data provided.', recommendation: 'Add electricity consumption from utility invoices. Required for SECR.' })
  if (scope1.filter(l => !l.input.notes?.startsWith('Auto WTT')).length === 0)
    gaps.push({ code: 'MISSING_SCOPE_1', severity: 'critical', message: 'No Scope 1 emissions data provided.', recommendation: 'Add natural gas or vehicle fuel data. Required for SECR.' })
  if (estimated_lines > 0)
    gaps.push({ code: 'ESTIMATED_DATA', severity: 'moderate', message: `${estimated_lines} line(s) use estimated quantities.`, recommendation: 'Replace estimates with actual meter reads before final disclosure.' })
  if (!intensity?.employees)
    gaps.push({ code: 'MISSING_INTENSITY', severity: 'minor', message: 'Intensity metrics not provided.', recommendation: 'Add employee count and revenue to calculate tCO₂e per employee and per £m revenue. Required for SECR.' })
  const hasCat1 = inputs.some(i => i.factor_id.startsWith('cat1_'))
  if (!hasCat1)
    gaps.push({ code: 'MISSING_CAT1', severity: 'minor', message: 'No Scope 3 Category 1 (purchased goods & services) data provided.', recommendation: 'Consider adding spend-based Cat 1 data. Required for UK SRS and recommended for comprehensive SECR disclosure.' })
  ...
  return {
    ...
    gaps,
    metadata: { ghg_protocol_consolidation: 'operational_control', scope_2_method: 'location_based', factor_version_provisional: isProvisional(factor_version_year) },
  }
}
